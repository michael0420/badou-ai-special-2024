...
def nearest_interp(img):
    ...
    emptyImage = np.zeros((800, 800, channels), np.uint8)  # 创建一个800*800的空白图像
    sh = 800 / height  # 计算高度的缩放比例
    sw = 800 / width  # 计算宽度的缩放比例
    for i in range(800):  # 遍历新图像的每个像素
        for j in range(800):
...
newImg = nearest_interp(img)  # 对图像进行最近邻插值
